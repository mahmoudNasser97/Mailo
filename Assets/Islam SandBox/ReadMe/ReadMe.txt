...
1. Create a Canvas with:
   ...
   - A TMP_Text named e.g. "MemberListLabel" (shows current lobby members)
   - Four Buttons: "Create Lobby", "Join by ID", "Leave Lobby", "Invite Friend"
2. Create an empty GameObject (e.g. "LobbyTest"), add the
   "Steam Lobby Debug Probe" component.
3. In the Inspector, drag each UI element into its matching field:
   _lobbyIdInputField, _statusLabel, _memberListLabel,
...
2. Click "Create Lobby".
   ...
   MemberListLabel shows just your own name.
3. Click "Leave Lobby".
   Expected Console: LobbyLeft. MemberListLabel clears.
...
4. Expected: both testers' MemberListLabel update to show both names shortly
   after (this comes through Steam's LobbyChatUpdate_t event, may take a
   short moment — not instant).
...
5. Both MemberListLabels update to show both names.
...
- Clicking Create/Join a second time before the first one finishes will
  silently replace the pending request (acceptable for this step's scope,
  not handled with a "please wait" lock yet).
